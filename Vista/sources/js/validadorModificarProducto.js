document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", (event) => {
        ...
        // Si NO subió archivo → es válido (no quiere cambiar la imagen)
        if (archivo === "") {
            // Limpio errores y NO marco verde (porque no es un campo obligatorio)
            imagen.classList.remove("border-red-500", "border-green-500");
            errorImagen.textContent = "";
        } 
        // Si subió archivo → validar extensión
        else {
            campoImagenValido = validarCampoImagen(archivo);
            if (campoImagenValido != "") {
                marcarError(imagen, errorImagen, campoImagenValido);
                valido = false;
            } else {
                marcarValido(imagen, errorImagen);
